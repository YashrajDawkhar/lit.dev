...
export class ProductImageSlider extends TailwindElement {
    ...
    render() {
        return html`
            <div class="max-w-6xl mx-auto p-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20">
                <div class="flex flex-col gap-5">
                    <div class="relative w-full aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-blue-50 to-cyan-50 shadow-xl border border-white/30 group">
                        <img 
                    ...
                    <div class="flex gap-3 overflow-x-auto scrollbar-thin scrollbar-track-slate-100 scrollbar-thumb-slate-300 p-2">
                        ${this.images.map((image, index) => html`
                            <div 
                                class="flex-shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden cursor-pointer border-3 transition-all duration-300 ease-out shadow-lg bg-white hover:border-purple-400 hover:shadow-xl hover:-translate-y-1 ${index === this.activeImageIndex ? 'border-purple-500 shadow-purple-300/30 -translate-y-1' : 'border-transparent'}"
                ...
                <div class="py-5">
                    <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent mb-5 leading-tight">Ottercat Cowboy Baby Toy</h1>
                    
                    <div class="flex items-center gap-4 mb-6 p-4 bg-gradient-to-r from-slate-50 to-blue-50 rounded-2xl border border-slate-200">
                        <span class="text-2xl font-bold bg-gradient-to-r from-green-600 to-emerald-500 bg-clip-text text-transparent">$1,299.00</span>
                        <span class="text-lg text-slate-400 line-through font-medium">$2,099.00</span>
                    </div>

                    <div class="flex items-center gap-3 mb-8 p-3 bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl border border-amber-200">
                        ...
                        <span class="text-amber-700 font-semibold">4.8 (128 reviews)</span>
                    </div>
                    ...
                    <div class="border-t-2 border-slate-200 pt-6 bg-gradient-to-r from-slate-50 to-blue-50 rounded-2xl p-5 mt-5">
                        <div class="mb-5">
                            <div class="flex justify-between items-center p-4 cursor-pointer font-semibold text-gray-700 bg-white rounded-xl transition-all duration-300 ease-out border border-slate-200 shadow-sm hover:text-gray-900 hover:bg-gradient-to-r hover:from-slate-50 hover:to-blue-50 hover:-translate-y-0.5 hover:shadow-md" @click="${() => this.toggleDetail('description')}">
                            ...
                            <div class="p-5 text-slate-600 text-sm leading-relaxed bg-white rounded-b-xl mt-0.5 border border-slate-200 border-t-0 ${this.openDetails.has('description') ? 'block animate-in slide-in-from-top-2 duration-300' : 'hidden'}">
                        ...
                        <div class="mb-0">
                            ...
                            <div class="p-5 text-slate-600 text-sm leading-relaxed bg-white rounded-b-xl mt-0.5 border border-slate-200 border-t-0 ${this.openDetails.has('specs') ? 'block animate-in slide-in-from-top-2 duration-300' : 'hidden'}">
...
export class AddToCartButton extends TailwindElement {
    ...
    render() {
        ...
        return html`
            <button 
                class="w-full py-5 px-6 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white border-none rounded-2xl text-lg font-bold cursor-pointer transition-all duration-300 ease-out mb-6 shadow-lg shadow-purple-500/30 relative overflow-hidden font-sans hover:-translate-y-0.5 hover:shadow-xl hover:shadow-purple-500/40 active:translate-y-0 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-gray-400/30 ${this.isSuccess ? 'from-green-600 to-green-700 hover:from-green-700 hover:to-green-800' : ''}"
    ...
    render() {
        ...
        return html`
            <div class="fixed top-5 right-5 left-5 md:left-auto z-50 font-sans">
                <div class="bg-gradient-to-r from-green-600 to-green-700 text-white p-5 rounded-2xl shadow-xl shadow-green-500/30 border border-white/20 backdrop-blur-xl max-w-sm mx-auto md:mx-0 animate-in slide-in-from-right-full scale-in-95 duration-500 ease-out relative overflow-hidden cursor-default">
                    <div class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-green-400 to-green-300 animate-pulse"></div>
